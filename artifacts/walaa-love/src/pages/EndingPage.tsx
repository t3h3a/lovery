  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center relative overflow-hidden px-4 py-8">
      ...
      <div
        ...
        style={{
          background: "radial-gradient(ellipse at 50% 50%, rgba(255,50,120,0.15) 0%, rgba(180,0,255,0.08) 50%, transparent 70%)",
        }}
      />
      ...
      <div className="absolute top-2 right-0 animate-float-bow opacity-70" style={{ animationDelay: "1s" }}>
        <Ribbon color="purple" />
      </div>
      ...
      <div className="relative z-10 text-center max-w-md">
        {/* Big Kitty */}
        <div
          className={`mb-6 transition-all duration-700 ${step >= 1 ? "opacity-100 scale-100" : "opacity-0 scale-90"}`}
        >
        ...
        {/* Main ending text */}
        <div
          className={`transition-all duration-700 mb-4 ${step >= 1 ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`}
        >
          <h2
        ...
        {/* Sub text */}
        <div
          className={`transition-all duration-700 mb-8 ${step >= 2 ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}
        >
          <p
        ...
        {/* Sparkle row */}
        <div
          className={`flex justify-center gap-3 mb-8 transition-all duration-700 ${step >= 3 ? "opacity-100" : "opacity-0"}`}
        >
          {["🌸", "✨", "💫", "✨", "🌸"].map((s, i) => (
            <span
              key={i}
        ...
        {/* Replay button */}
        <div
          className={`transition-all duration-700 ${step >= 3 ? "opacity-100 scale-100" : "opacity-0 scale-90"}`}
        >
          <button
            data-testid="button-replay"
            onClick={onReplay}
            className="btn-glow text-white font-bold rounded-full px-8 py-4 text-xl flex items-center gap-2 mx-auto"
        ...
        {/* Final note */}
        <div
          className={`mt-4 transition-all duration-700 ${step >= 3 ? "opacity-100" : "opacity-0"}`}
        >
          <p
